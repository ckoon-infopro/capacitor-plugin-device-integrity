...
public class InternalRootDetection {
    ...
    public boolean isRooted(final Context context) {
        ...
        Logger.debug(Constants.LOG_TAG, "check c1 = isExistBuildTags: " + c1);
        Logger.debug(Constants.LOG_TAG, "check c2 = doesSuperuserApkExist: " + c2);
        Logger.debug(Constants.LOG_TAG, "check c3 = isExistSUPath: " + c3);
        Logger.debug(Constants.LOG_TAG, "check c4 = checkDirPermissions: " + c4);
        Logger.debug(Constants.LOG_TAG, "check c5 = checkExecutingCommands: " + c5);
        Logger.debug(Constants.LOG_TAG, "check c6 = checkInstalledPackages: " + c6);
        Logger.debug(Constants.LOG_TAG, "check c7 = checkforOverTheAirCertificates: " + c7);
        Logger.debug(Constants.LOG_TAG, "check c8 = isRunningOnEmulator: " + c8);

        boolean result = c1 || c2 || c3 || c4 || c5 || c6 || c7 || c8;

        Logger.debug(Constants.LOG_TAG, String.format("[isRooted] result: %s", result));
    ...
    private boolean checkDirPermissions() {
        ...
        Logger.debug(Constants.LOG_TAG, String.format("[checkDirPermissions] result: %s", result));
    ...
    private boolean isExistBuildTags() {
        ...
        try {
            ...
            // Logger.debug(Constants.LOG_TAG, String.format("[isExistBuildTags] buildTags: %s", buildTags));

            result = (buildTags != null) && buildTags.contains("test-keys");
        } catch (Exception e) {
            Logger.error(Constants.LOG_TAG, String.format("[isExistBuildTags] Error: %s", e.getMessage()), e);
        }

        Logger.debug(Constants.LOG_TAG, String.format("[isExistBuildTags] result: %s", result));
    ...
    private boolean doesSuperuserApkExist() {
        ...
        for (String path : Constants.SUPER_USER_APK_FILES) {
            ...
            if (rootFile.exists()) {
                Logger.debug(Constants.LOG_TAG, String.format("[doesSuperuserApkExist] found SU apk: %s", path));

                result = true;
            }
        }

        Logger.debug(Constants.LOG_TAG, String.format("[doesSuperuserApkExist] result: %s", result));
    ...
    private boolean isExistSUPath() {
        ...
        for (final String path : pathsArray) {
            ...
            if (fileExists) {
                Logger.debug(Constants.LOG_TAG, String.format("[isExistSUPath] binary [%s] detected!", path));

                result = true;
            }
        }

        Logger.debug(Constants.LOG_TAG, String.format("[isExistSUPath] result: %s", result));
    ...
    private boolean checkInstalledPackages(final Context context) {
        ...
        Logger.debug(Constants.LOG_TAG, String.format("[checkInstalledPackages] count of root-only apps: %s", rootOnlyAppCount));

        boolean result = rootOnlyAppCount > 2; // todo: why?

        Logger.debug(Constants.LOG_TAG, String.format("[checkInstalledPackages] result: %s", result));
    ...
    private boolean checkforOverTheAirCertificates() {
        ...
        Logger.debug(Constants.LOG_TAG, String.format("[checkforOverTheAirCertificates] exist: %s", exist));
        Logger.debug(Constants.LOG_TAG, String.format("[checkforOverTheAirCertificates] result: %s", result));
    ...
    private boolean checkExecutingCommands() {
        ...
        Logger.debug(Constants.LOG_TAG, String.format("[checkExecutingCommands] result [%s] => [c1:%s][c2:%s][c3:%s]", result, c1, c2, c3));
